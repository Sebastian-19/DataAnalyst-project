...
update Circuitos set Pais1 = español from paises right join Circuitos on Circuitos.Pais = paises.Ingles;
...
-- elimino la tabla temporal 'Paises'
...
drop table Nacionalidad_constructores;
...
update Pilotos set Numero = REPLACE('\N', Numero, NULL) where Numero = '\N';
...
-- Elimino la columna antigua [Nacionalidad] y la tabla temporal 'PilotosNac'. Tambien cambio el nombre de la columna nueva de [n1] a [Nacionalidad]

Alter table pilotos
	drop column Nacionalidad;

drop table pilotosnac;
...
-- Elimino la columna [tiempo] y creo una nueva con los datos correspondientes, llamada [t1] que voy a cambiar a [tiempo]
	...
	update Resultados set T1 = case	
									when Tiempo like '%"%' then REPLACE(Tiempo, '"', '') 
...
update Resultados set [rank] = REPLACE([rank], '\N', null) where  [rank] = '\N'; 
...
update Resultados set [VueltaRapida] = REPLACE([VueltaRapida], '\N', null) where  [VueltaRapida] = '\N'; 
...
update Resultados set VueltaRapida_Tiempo = case
												when VueltaRapida_Tiempo like '%"%' 
													then REPLACE(VueltaRapida_Tiempo, '"', '')
												when VueltaRapida_Tiempo = '\N'
													then NULL
											END
;
...
update Resultados set vueltarapida_velocidad = case
												when vueltarapida_velocidad like '%"%' 
													then REPLACE(vueltarapida_velocidad, '"', '')
												when vueltarapida_velocidad = '\N'
													then NULL
											END
;
...
-- '\N' a NULL.
update Resultados_constructores set estado = REPLACE('\N', estado, null) where Estado = '\N';

-- 'D' a 'Descalificado'.
update resultados_constructores set estado = REPLACE('D', estado, 'Descalificado') where Estado = 'D';
...
update Resultados_Sprint set Tiempo = REPLACE('\N', tiempo, null) where Tiempo = '\N';
update Resultados_Sprint set VueltaRapida = REPLACE('\N', VueltaRapida, null) where VueltaRapida = '\N';
update Resultados_Sprint set VueltaRapidaTiempo = REPLACE('\N', VueltaRapidaTiempo, null) where VueltaRapidaTiempo= '\N';
...
alter table temporadas
	alter column año smallint NOT NULL;

-- Elimino la columna URL que no va a ser de utilidad
...
-- creo las llaves primarias de todas las tablas
	...
	alter table Temporadas
		add constraint Pkaño primary key ([Año]);
...
-- Creo las llaves foraneas 
	...
	alter table gp 
		add constraint FKaño foreign key (Año) references temporadas(año);
	alter table gp
